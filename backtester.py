"""
티커를 입력받아 SOXL 백테스터 인프라를 활용해 손쉽게 백테스트를 실행합니다.

`soxl_quant_system.SOXLQuantTrader`는 내부적으로 SOXL 심볼을 사용하도록
구현되어 있으므로, 이 모듈에서는 해당 호출을 원하는 티커로 매핑하는
작은 어댑터 클래스를 제공합니다.
"""
...
class TickerMappedTrader(SOXLQuantTrader):
    """
    내부적으로 SOXL을 참조하는 `SOXLQuantTrader` 호출을 지정한 티커로
    매핑해 주는 래퍼 클래스.
    """
    ...
    def get_stock_data(self, symbol: str, period: str = "1mo"):
        # 기존 로직에서 SOXL을 요청하면 사용자가 입력한 티커로 대체
...
def run_backtest(
    ...
    *,
    ...
    """
    원하는 티커로 SOXL 백테스팅 로직을 재사용하는 헬퍼 함수.

    Parameters
    ----------
    start_date:
        백테스트를 시작할 날짜(YYYY-MM-DD).
    end_date:
        백테스트를 종료할 날짜. `None`이면 오늘 날짜(또는 장 마감 시각)에 맞춰 자동 결정됩니다.
    initial_capital:
        초기 투자금. 기본값은 40,000달러입니다.
    symbol:
        백테스트에 사용할 티커. 기본값은 "SOXL".
    quiet:
        True로 설정하면 내부 출력(log)을 최소화합니다.

    Returns
    -------
    Dict
        `SOXLQuantTrader.run_backtest`가 반환하는 요약 정보를 그대로 돌려줍니다.
    """
    symbol = symbol.upper()
    trader_class = SOXLQuantTrader if symbol == "SOXL" else TickerMappedTrader

    if trader_class is TickerMappedTrader:
        trader = trader_class(symbol, initial_capital)
    else:
        trader = trader_class(initial_capital)

    if quiet:
        # 시뮬레이션 캐시를 활용하기 위해 session_start_date를 설정한 뒤,
        # `simulate_from_start_to_today`를 호출하는 루틴을 그대로 사용합니다.
        trader.session_start_date = start_date
        result = trader.simulate_from_start_to_today(start_date, quiet=True)
        if result.get("skipped"):
            # 최신 거래일이 start_date보다 이전인 경우 run_backtest가 수행되지 않으므로,
            # 필요한 최소 정보만 반환합니다.
...
def _calculate_mdd(daily_records: List[Dict]) -> Dict:
    """
    일별 기록으로부터 MDD(Most Drawdown) 정보를 계산한다.
...
def export_backtest_to_excel(result: Dict, filename: Optional[str] = None) -> Optional[Path]:
    ...
    if filename is None:
        ...
        filename = f"{symbol}_백테스팅_{result.get('start_date', 'unknown')}_{timestamp}.xlsx"
