    ...
    if st.session_state.trader is None:
        try:
            with st.spinner('MOS 퀀트투자 시스템 초기화 중...'):
                ...
                st.session_state.trader = SHNYQuantTrader(
            ...
            with col3:
                if st.button("🗑️", key=f"delete_seed_{i}"):
                    ...
                    if st.session_state.get("active_preset"):
                        save_preset_snapshot_shny(st.session_state.active_preset, {})
                    st.session_state.trader = None
                    st.rerun()
...
def show_dashboard():
    ...
    if latest_trading_day.strftime('%Y-%m-%d') == '2025-10-10':
        ...
        # SHNY 데이터 가져오기
        soxl_data = st.session_state.trader.get_stock_data("SHNY", "1mo")
        if soxl_data is not None and len(soxl_data) > 0:
            # 디버깅: 데이터 범위 확인
            st.info(f"📊 SHNY 데이터 범위: {soxl_data.index[0].strftime('%Y-%m-%d')} ~ {soxl_data.index[-1].strftime('%Y-%m-%d')}")
...
def show_daily_recommendation():
    ...
    with col2:
        ...
        st.metric("💰 SHNY 현재가", f"${recommendation['soxl_current_price']:.2f}")
    ...
    with col1:
        ...
        if recommendation['can_buy']:
            ...
            st.info(
                f"💵 매수금액: \\${recommendation['next_buy_amount']:,.0f}  ·  "
                f"📐 투자원금(갱신 기준) \\${inv_base:,.0f} × 비중 {buy_ratio_pct:.1f}% "
    ...
    if 'backtest_result' in st.session_state and st.session_state.backtest_result:
        ...
        if st.button("📥 엑셀 파일 생성", key="generate_excel"):
            with st.spinner('엑셀 파일 생성 중...'):
                ...
                temp_filename = f"SHNY_백테스팅_{backtest_result['start_date']}_{timestamp}.xlsx"
                ...
                if result_filename and os.path.exists(result_filename):
                    ...
                    st.download_button(
                        label="💾 엑셀 파일 다운로드",
                        data=excel_data,
                        file_name=f"SHNY_백테스팅_{backtest_result['start_date']}_{timestamp}.xlsx",
