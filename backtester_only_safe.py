...
class SafeOnlyTrader(SOXLQuantTrader):
    """
    AG(공세) 모드를 사실상 비활성화하여 SF(안전) 모드만 동작하도록 하는 트레이더.
    - 매우 비현실적인 AG 매수/매도 임계값으로 설정하여 AG 진입 자체가 발생하지 않도록 함.
    """

    def __init__(self, initial_capital: float = 40_000, sf_config: Optional[Dict] = None):
        # 사용자가 sf_config를 넘기면 반영, 그렇지 않으면 기본값은 SOXLQuantTrader의 기본을 사용
        safe_config = sf_config.copy() if sf_config is not None else None

        # AG 모드는 진입이 불가능하도록 극단적으로 높게 설정
        disabled_ag_config = {
            "buy_threshold": 999.0,
        ...
        super().__init__(initial_capital=initial_capital, sf_config=safe_config, ag_config=disabled_ag_config)


class TickerMappedSafeTrader(SafeOnlyTrader):
    """
    내부적으로 SOXL을 참조하는 호출을 지정한 티커로 매핑하고,
    동시에 안전모드만 동작하도록 하는 래퍼 클래스.
    """
...
def run_backtest(
    ...
    *,
    ...
    """
    안전(SF) 모드만으로 백테스트를 수행하는 헬퍼 함수.
    """
    symbol = symbol.upper()
    trader_class = SafeOnlyTrader if symbol == "SOXL" else TickerMappedSafeTrader

    if trader_class is TickerMappedSafeTrader:
        trader = trader_class(symbol, initial_capital=initial_capital, sf_config=sf_config)
    else:
        trader = trader_class(initial_capital=initial_capital, sf_config=sf_config)
...
def export_backtest_to_excel(result: Dict, filename: Optional[str] = None) -> Optional[Path]:
    ...
    if filename is None:
        ...
        filename = f"{symbol}_백테스팅_SF전용_{result.get('start_date', 'unknown')}_{timestamp}.xlsx"
